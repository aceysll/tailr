...
export default function Tailor() {
  ...
  const handleFile = async (file) => {
    ...
    if (file.type === "application/pdf") {
      try {
        ...
        if (!fullText.trim()) {
          setPdfError("Could not extract text. This PDF may be scanned. Please paste your CV as text.")
          return
        }
        setCv(fullText.trim())
        setPdfError("")
      } catch (e) {
        setPdfError("Could not read this PDF. Please paste your CV as text instead.")
      }
      return
    }
  ...
  const generate = async () => {
    ...
    if (!canGenerate) {
      setError("You have used all 3 free tailorings for today. Come back tomorrow.")
      return
    }
    setLoading(true)
    setError("")
    ...
    <div style={{ minHeight: "100vh", background: T.bg, color: T.text, fontFamily: T.sans }}>
      <nav style={{
        ...
        <button onClick={() => navigate('/')} style={{
          ...
          color: T.amber, background: "none", border: "none", cursor: "pointer",
        }}>Tailr</button>
        <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
          <div style={{ fontSize: 12, color: remaining > 0 ? T.amber : T.danger, fontWeight: 600 }}>
      ...
      {loading && (
        <div style={{
          ...
          <div style={{ fontFamily: T.serif, fontSize: 48, fontWeight: 700, fontStyle: "italic", color: T.amber, marginBottom: 16 }}>
            Tailoring...
          </div>
          <div style={{ fontSize: 14, color: T.textDim, maxWidth: 300, textAlign: "center", lineHeight: 1.7 }}>
            Reading your CV and the job description. Writing your tailored bullets and cover letter.
          </div>
        </div>
      )}
      ...
      <div style={{ maxWidth: 1100, margin: "0 auto", padding: "32px 24px" }}>
        <div style={{ marginBottom: 28 }}>
          <h1 style={{ fontFamily: T.serif, fontSize: 32, fontWeight: 700, letterSpacing: "-0.02em", marginBottom: 6 }}>
        ...
        {error && (
          <div style={{ padding: "12px 16px", borderRadius: T.md, marginBottom: 20, background: "#1a0806", border: `1px solid ${T.danger}40`, fontSize: 13, color: T.danger }}>
            {error}
          </div>
        )}
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
              ...
              <div style={{ display: "flex", gap: 8 }}>
                <button onClick={() => fileRef.current?.click()} style={{ padding: "4px 10px", borderRadius: T.sm, background: T.surface, border: `1px solid ${T.borderHi}`, color: T.textDim, fontSize: 11, fontWeight: 600, cursor: "pointer" }}>
                  Upload PDF or TXT
                </button>
                <button onClick={() => setCv("")} style={{ padding: "4px 10px", borderRadius: T.sm, background: "transparent", border: `1px solid ${T.border}`, color: T.textMuted, fontSize: 11, cursor: "pointer" }}>Clear</button>
              </div>
            </div>
            <input ref={fileRef} type="file" accept=".pdf,.txt" style={{ display: "none" }} onChange={(e) => handleFile(e.target.files[0])} />
            {pdfError && (
              <div style={{ fontSize: 11, color: T.amber, padding: "8px 12px", borderRadius: T.sm, background: T.amberDim, border: `1px solid ${T.amber}30`, lineHeight: 1.5 }}>
                {pdfError}
              </div>
            )}
