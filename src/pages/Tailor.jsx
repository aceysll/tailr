    ...
    <div style={{ minHeight: "100vh", background: T.bg, color: T.text, fontFamily: T.sans }}>
      <nav style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "20px 32px", borderBottom: `1px solid ${T.border}`, position: "sticky", top: 0, zIndex: 10, background: T.bg }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
          ...
          <button onClick={generate} disabled={loading || !canGenerate} style={{ padding: "9px 20px", borderRadius: T.md, border: "none", background: loading || !canGenerate ? T.surfaceHi : T.amber, color: T.bg, fontSize: 13, fontWeight: 700, cursor: "pointer", fontFamily: T.sans }}>{loading ? "Tailoring..." : "Tailor my CV"}</button>
        </div>
      </nav>
      ...
      {loading && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(13,13,13,0.92)", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", zIndex: 100 }}>
          ...
          <div style={{ fontSize: 14, color: T.textDim, maxWidth: 300, textAlign: "center" }}>Reading your CV and the job description.</div>
        </div>
      )}
